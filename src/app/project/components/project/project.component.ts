...
export class ProjectComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    if (this.router.url.indexOf('invite') > -1) {
      ...
      this.inviteKey = toString(this.route.snapshot.paramMap.get('key')).trim();
    } else {
  ...
  ngAfterViewInit(): void {
    const open$ = fromEvent(this.canvasDiv.nativeElement, 'click')
      .pipe(
        filter((event: any) => this.canvasDiv.nativeElement === event['target'])
      )
      .subscribe((e: any) => {
    ...
    this.uploadNameSubject
      ...
      .subscribe((resp) => {
        ...
        this.uploadService
          ...
          .then(() => (this.projectData.uploads[resp.index].isLoading = false))
          .catch((err) => {
            console.log(err);
            this.snackBar.snackbarError('Error while updating name!');
          });
      });
  }
    ...
    this.uploadService
      ...
      .subscribe((data) => {
        this.projectData.uploads = data
          ...
          .filter((x) => x.projectKey === this.projectKey && x.isActive);
    ...
    this.commentService
      ...
      .subscribe((data) => {
        const _commentsInProject = data
          ...
          .filter((x) => x.projectKey === this.projectKey && x.status !== 0);

        this.projectData.commentsInProject = _commentsInProject.map((c) => {
        ...
        this.projectData.currentComments =
          this.projectData.commentsInProject.filter(
            (o: any) =>
              o.imageKey === this.projectData.selectedImg.key && o.status !== 0
          );
        ...
        this.panelData.files.map((file: any) => {
          file.comments = [];
          file.activeComments = this.projectData.commentsInProject.filter(
            (p: any) => p.imageKey === file.key && p.status === 1
          );
          file.resolvedComments = this.projectData.commentsInProject.filter(
            (p: any) => p.imageKey === file.key && p.status === 2
          );
        });
        this.retrieveUsers();
      });
    ...
    this.projectService
      ...
      .subscribe((data) => {
        const temp = data
          ...
          .filter(
            (x) =>
              (x.key === this.projectKey || x.inviteLink === this.inviteKey) &&
              x.isActive
          )[0];
        if (temp) {
          ...
          if (this.isInviteLink) {
            ...
            const updated = projectUtil.makeUpdatedProjectInvite(temp);
            ...
            if (allowProject?.createdBy || allowProject?.sharedWithUser) {
              this.projectData.project = temp;
  ...
  retrieveUsers(): void {
    this.userService
      ...
      .subscribe((data) => {
        this.users = data.map((c) => new UserViewModel().deserialize(c));
              ...
              if (c.children) {
                c.children.map((child: any) => {
                  const childUser = this.users.filter(
                    (u) =>
                      u.userKey === child.createdBy || u.key === child.createdBy
                  )[0];
                  const childDisplayName =
                    childUser.username || childUser.fullName;
                  child.displayName = toString(childDisplayName).trim();
                  child.isEdit = false;
                  return child;
                });
              }
              const user = this.users.filter(
                (u) => u.userKey === c.createdBy || u.key === c.createdBy
              )[0];
        ...
        if (this.panelData.comment2edit) {
          const _user = this.users.filter(
            (u) =>
              u.userKey === this.panelData.comment2edit.createdBy ||
        ...
        if (this.panelData.files) {
          this.panelData.files.map((file: any) => {
            if (file.activeComments) {
              file.activeComments = file.activeComments.map((ac: any) => {
                const user = this.users.filter(
                  (u) => u.userKey === ac.createdBy || u.key === ac.createdBy
                )[0];
                const displayName = user.username || user.fullName;
                const temp = {
                  ...cloneDeep(ac),
            ...
            if (file.resolvedComments) {
              file.resolvedComments = file.resolvedComments.map((rs: any) => {
                const user = this.users.filter(
                  (u) => u.userKey === rs.createdBy || u.key === rs.createdBy
                )[0];
                const displayName = user.username || user.fullName;
                const temp = {
                  ...cloneDeep(rs),
        ...
        if (this.panelData.comment2edit) {
          const updatedComment = this.projectData.commentsInProject.filter(
            (x: any) => x.key === this.panelData.comment2edit.key
          );
          this.panelData.comment2edit = updatedComment[0];
        }
        if (this.projectData?.project) {
          this.projectUsers = (this.projectData?.project.sharedUsers || []).map(
            (user: any) => {
              const info = this.users.filter((u) => {
                return u.userKey === user;
              });
              return info ? (info[0] ? info[0].fullName : null) : null;
            }
          );
        }
      });
  ...
  nextImg(): void {
    if (this.projectData.imgIndex < this.projectData.uploads.length - 1) {
      ...
      this.projectData.currentComments =
        this.projectData.commentsInProject.filter(
          (x: any) =>
            x.imageKey === this.projectData.selectedImg.key && x.status !== 0
        );
      setTimeout(() => {
        this.onResized(null);
      }, 100);
  ...
  replaceImg(event: any, picture: any, index: number): void {
    ...
    img.onload = (e: any) => {
      const height = e.currentTarget.naturalHeight || e.currentTarget.height;
  ...
  changeCollapsible(i: number): void {
    this.panelData.files[i].isCollapsed = !this.panelData.files[i].isCollapsed;
  }
  resolveComment(comment: any): void {
    ...
    if (comment && key !== '') {
      ...
      this.projectService.updateNewTags(this.projectData.project.key, newTags);
