...
export class DashboardComponent implements OnInit, AfterViewInit {
    ...
    this.projectNameSubject
      ...
      .subscribe((resp) => {
        ...
        this.projectService
          ...
          .catch((err) => {
            this.snackBar.snackbarError('Unable to update project name!');
          });
      });
  }
  ...
  retrieveFiles(): void {
    this.uploadService
      ...
      .subscribe((data) => {
        this.uploads = data.map((c) => new PictureViewModel().deserialize(c));
        this.projects.map((proj) => {
          const pictures = this.uploads.filter(
            (up) => up.projectKey === proj.key && proj.isActive && up.isActive
          );
  ...
  openShare(project: any): void {
    ...
    this.projectService.updateInviteLink(this.project2share?.key, link?.key);
