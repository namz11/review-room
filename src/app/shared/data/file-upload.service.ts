...
export class FileUploadService {
            ...
            if (isReplace) {
              ...
              this.uploadsRef.push(fileUpload.data).then((x) => {
                const replaceData = {
                  ...
                  comments2replace: cloneDeep(allowReplace?.comments2replace),
              ...
              if (data.isActive) {
                this.projectService.updateUploadCount(data.projectKey, 1);
              }
            }
          });
        })
  ...
  private replaceComments(replaceData: any): void {
    ...
    replaceData.comments2replace.map((c: any) => {
      const newX =
        (c.x / toNumber(replaceData.oldImg?.width)) * replaceData.newWidth;
      const newY =
        (c.y / toNumber(replaceData.oldImg.height)) * replaceData.newHeight;
