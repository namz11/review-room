...
export const generalUtil = {
  ...
  getCurrentUserEmail: () => {
    const details = JSON.parse(toString(localStorage.getItem('userDetails')));
...
export const projectUtil = {
    ...
    if (project?.allowedEmails) {
      if (project?.allowedEmails.length > 0) {
        flag.sharedWithEmail =
          (project?.allowedEmails as string[]).includes(email.trim()) &&
          project.isActive;
      }
    }
    return flag;
  ...
  allowProject: (
    ...
    if (project.createdBy === uid && project.isActive) {
      return { createdBy: true, sharedWithUser: false, sharedWithEmail: false };
    } else {
